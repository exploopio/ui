...
import { Loader2, Bell, MessageSquare, Send, Webhook, ChevronDown, Settings2, Mail } from "lucide-react";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
export function AddNotificationDialog({
  ...
  // Get enabled modules for filtering event types based on tenant's plan
  const { moduleIds: enabledModuleIds } = useTenantModules();

  // Filter event types based on enabled modules
  const availableEventTypes = useMemo(
    () => getAvailableEventTypes(enabledModuleIds),
    [enabledModuleIds]
  );

  // Get default event types filtered by available modules
  const defaultEventTypes = useMemo(
    () => getDefaultEnabledEventTypes(enabledModuleIds),
    [enabledModuleIds]
  );
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      await createIntegration({
        ...
        // Event type filters (dynamic JSONB array) - cast to expected type
        enabled_event_types: data.enabled_event_types as NotificationEventType[],
        // Advanced settings
      ...
      toast.error(error instanceof Error ? error.message : "Failed to create notification channel");
    }
  };
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {step === "provider" ? "Add Notification Channel" : `Configure ${selectedProvider?.name}`}
          </DialogTitle>
          <DialogDescription>
            {step === "provider"
        ...
        {step === "provider" ? (
          <div className="grid grid-cols-2 gap-3 py-4">
            {PROVIDERS.map((provider) => {
              ...
              return (
                ...
                >
                  <div className={cn("flex h-12 w-12 items-center justify-center rounded-lg", provider.bgColor)}>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 min-h-0">
            ...
            {selectedProvider?.id === "email" && (
              <div className="space-y-4 rounded-lg border p-4 bg-muted/30">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="smtp_port">Port <span className="text-destructive">*</span></Label>
                    <Input
                      id="smtp_port"
                      type="number"
                      placeholder="587"
                ...
                <div className="space-y-2">
                  <Label htmlFor="to_emails">Recipients <span className="text-destructive">*</span></Label>
                  <Input
                    id="to_emails"
            ...
            {selectedProvider?.id === "telegram" && (
              <div className="space-y-2">
                <Label htmlFor="channel_id">Chat ID <span className="text-destructive">*</span></Label>
                <Input
                  id="channel_id"
                ...
                <p className="text-xs text-muted-foreground">
                  Get your chat ID: 1) Add @userinfobot to your group/channel, 2) It will reply with the chat ID.
                  For groups/channels, use the numeric ID (e.g., -1001234567890).
                </p>
              </div>
            )}
                      ...
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            ...
                            setValue("enabled_severities", enabledSeverities.filter((s: NotificationSeverity) => s !== severity.value));
                          }
                        }}
                      />
                      <label htmlFor={`severity_${severity.value}`} className={cn("text-sm font-normal cursor-pointer", colorClass)}>
                        {severity.label}
                      </label>
                    </div>
                  );
            ...
            <div className="space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                {availableEventTypes.length === 0 && " (loading...)"}
              </p>
              <div className="grid grid-cols-2 gap-3">
                {availableEventTypes.map((eventType) => {
                  const enabledTypes = watch("enabled_event_types");
                  const isChecked = enabledTypes.includes(eventType.value);
                  return (
                    <div key={eventType.value} className="flex items-center space-x-2">
                      <Checkbox
                        id={`event_${eventType.value}`}
                        checked={isChecked}
                        onCheckedChange={(checked) => {
                          if (checked) {
                            setValue("enabled_event_types", [...enabledTypes, eventType.value]);
                          } else {
                            setValue("enabled_event_types", enabledTypes.filter((t) => t !== eventType.value));
                          }
                        }}
                      />
                      <label htmlFor={`event_${eventType.value}`} className="text-sm font-normal cursor-pointer">
                        {eventType.label}
                      </label>
                    </div>
                  );
            ...
            <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
              ...
              <CollapsibleContent className="space-y-4 pt-4">
                <div className="space-y-3">
                  ...
                  <Tabs value={templateTab} onValueChange={(v) => setTemplateTab(v as "edit" | "preview")}>
                ...
                <div className="space-y-3">
                  ...
                  />
                  <p className="text-xs text-muted-foreground">
                    Minimum interval between notifications (0 = no limit). Prevents notification spam.
                  </p>
                </div>
              </CollapsibleContent>
            </Collapsible>
