...
import { Loader2, Bell, MessageSquare, Send, Webhook, ChevronDown, Settings2, History } from "lucide-react";
import {
  Collapsible,
  CollapsibleContent,
  CollapsibleTrigger,
...
const PROVIDER_CONFIG: Record<string, { icon: typeof Bell; color: string; bgColor: string; credentialLabel: string }> = {
  slack: {
    icon: MessageSquare,
    color: "text-[#4A154B]",
...
export function EditNotificationDialog({
  ...
  // Get default event types filtered by available modules (used as fallback)
      ...
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <div className={cn("flex h-8 w-8 items-center justify-center rounded-lg", providerConfig.bgColor)}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="space-y-2">
            <Label htmlFor="credentials">{providerConfig.credentialLabel} (leave empty to keep current)</Label>
            <Input
              id="credentials"
              type="password"
          ...
          {integration.provider === "telegram" && (
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Get your chat ID: Add @userinfobot to your group/channel. For groups/channels, use the numeric ID (e.g., -1001234567890).
              </p>
            </div>
          )}
              ...
              {ALL_NOTIFICATION_SEVERITIES.map((severity) => {
                ...
                return (
                  <div key={severity.value} className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor={`edit_severity_${severity.value}`} className={cn("text-sm font-normal cursor-pointer", colorClass)}>
                      {severity.label}
                    </label>
                  </div>
                );
              ...
              {availableEventTypes.map((eventType) => {
                ...
                return (
                  <div key={eventType.value} className="flex items-center space-x-2">
                    <Checkbox
                      id={`edit_event_${eventType.value}`}
                      checked={isChecked}
                      onCheckedChange={(checked) => {
                        if (checked) {
                    ...
                    />
                    <label htmlFor={`edit_event_${eventType.value}`} className="text-sm font-normal cursor-pointer">
                      {eventType.label}
                    </label>
                  </div>
                );
          ...
          <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen}>
            ...
            <CollapsibleContent className="space-y-4 pt-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                />
                <label htmlFor="edit_include_details" className="text-sm font-normal cursor-pointer">
