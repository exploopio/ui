...
export const getAssetRelationships = (assetId: string): AssetRelationship[] =>
  assetRelationships.filter(
    (rel) => rel.sourceAssetId === assetId || rel.targetAssetId === assetId
  );
...
export const buildRelationshipGraph = (assetId?: string): RelationshipGraph => {
  const relevantRelationships = assetId
...
export const attackPaths: AttackPath[] = [
  ...
  },
  {
    ...
    description:
      "Attack path from vulnerable container image to production Kubernetes workloads",
    steps: [
      {
        assetId: "image-003",
  ...
  },
  {
    ...
    description:
      "Attack path from mobile application through payment API to transaction database",
    steps: [
      {
        assetId: "mobile-001",
