...
function getPermissionType(permissionId: string): string {
  ...
  if (permissionId.includes(':write') || permissionId.includes(':create') || permissionId.includes(':update') || permissionId.includes(':manage') || permissionId.includes(':assign') || permissionId.includes(':trigger') || permissionId.includes(':cancel') || permissionId.includes(':schedule') || permissionId.includes(':invite') || permissionId.includes(':bulk') || permissionId.includes(':export')) return 'write';
  return 'read';
}
...
export function RoleDetailSheet({ role, open, onOpenChange, onEdit, onDelete }: RoleDetailSheetProps) {
  ...
  const handleRemoveMember = useCallback(async (member: RoleMember) => {
    if (!role) return;
    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      <SheetContent className="sm:max-w-3xl p-0 flex flex-col h-full max-h-screen">
        ...
        <SheetHeader className="px-6 py-4 border-b shrink-0 pr-12">
          ...
          {!role.is_system && onEdit && onDelete && (
            <div className="flex items-center gap-2 mt-3 pt-3 border-t">
              ...
              <Button variant="outline" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => onDelete(role)}>
        ...
        <Tabs defaultValue="permissions" className="flex-1 flex flex-col min-h-0">
          ...
          {/* Permissions Tab */}
          <TabsContent value="permissions" className="flex-1 flex flex-col min-h-0 mt-0 pt-4 data-[state=inactive]:hidden">
            {/* Permission Type Legend */}
            <div className="px-6 pb-4 shrink-0">
              <div className="flex items-center gap-4 text-xs">
                <span className="text-muted-foreground">Types:</span>
                <div className="flex items-center gap-1">
                  <div className={cn('px-2 py-0.5 rounded', permissionTypeConfig.read.bgColor, permissionTypeConfig.read.color)}>
                    <Eye className="inline h-3 w-3 mr-1" />
                    Read ({permissionStats.read})
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <div className={cn('px-2 py-0.5 rounded', permissionTypeConfig.write.bgColor, permissionTypeConfig.write.color)}>
                    <Pencil className="inline h-3 w-3 mr-1" />
                    Write ({permissionStats.write})
                  </div>
                </div>
                <div className="flex items-center gap-1">
                  <div className={cn('px-2 py-0.5 rounded', permissionTypeConfig.delete.bgColor, permissionTypeConfig.delete.color)}>
          ...
          {/* Members Tab */}
          <TabsContent value="members" className="flex-1 flex flex-col min-h-0 mt-0 pt-4 data-[state=inactive]:hidden">
            ...
            <div className="flex-1 min-h-0 overflow-hidden">
              ...
              {isLoadingMembers ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  {!memberSearchQuery && (
                    <Button variant="outline" size="sm" className="mt-4" onClick={() => setAddMemberDialogOpen(true)}>
                      <UserPlus className="mr-1.5 h-4 w-4" />
                      Add First Member
                    </Button>
                  )}
