...
import { Link2, Copy, Eye, ExternalLink, Plus } from "lucide-react";
...
export function RelatedTab({ finding }: RelatedTabProps) {
  ...
  const totalRelated =
    similarFindings.length + linkedFindings.length + sameCveFindings.length;
      ...
      <div>
        <div className="mb-3 flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              Manually linked related findings
            </p>
          </div>
          <Button size="sm" variant="outline">
            <Plus className="mr-2 h-3 w-3" />
        ...
        <RelatedFindingsTable
          ...
          emptyMessage="No linked findings"
        />
      </div>
