...
/**
 * Set Keycloak access token (in-memory preferred, but can use short-lived cookie)
 * ⚠️ Recommendation: Store access tokens in memory (Zustand) instead
 */
...
/**
 * Set Keycloak refresh token (HttpOnly, long-lived)
...
/**
 * Get Keycloak access token from cookie
 */
