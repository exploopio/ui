...
export const findingKeys = {
  ...
  list: (filters?: FindingListFilters) =>
    [...findingKeys.lists(), filters] as const,
  details: () => [...findingKeys.all, "detail"] as const,
  detail: (id: string) => [...findingKeys.details(), id] as const,
  comments: (findingId: string) =>
    [...findingKeys.all, "comments", findingId] as const,
...
export function useAssetFindings(
  ...
  const key = currentTenant && assetId
    ? findingEndpoints.listByAsset(assetId, filters)
    : null;
...
export async function invalidateFindingsCache() {
  ...
  await mutate(
    (key) => typeof key === "string" && key.includes("/api/v1/findings"),
    undefined,
    { revalidate: true }
  );
}
